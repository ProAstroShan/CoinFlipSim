...
def start_game():
  ...
  confirm_sum = 0
  while confirm_sum < 5:
    print(intro[confirm_sum])
    go_on = input("\nPress any key to continue, type SKIP to skip dialouge\nand type quit to exit: ")
    if go_on in ["q","Q","Quit","quit"]:
      print("You quit the game! ")
      confirm = False
    else:
        ...
        if len(has_save) == 1 and has_save in ["1","2"]:
          ...
          if has_save == "1":
            if download(coin) == False:
              print("Save File was not found or corrupted, create a new one! ")
            else:
...
def download(coin):
  ...
  try:
    ...
    coin.money = round(save_data["money"],2)
...
class MasterCoin:
  ...
  def upload(self):
    backup_data = {
      "version": 1,
      "flip_time_level": self.flip_time_level,
